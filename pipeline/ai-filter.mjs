...
AI相关性硬规则（2026-08-13事故：CRISPR小鼠性别转换这类纯生物新闻混入AI资讯）：
- 与AI无实质关联的内容（纯生物/医学/农业/能源/汽车/金融/消费/体育/娱乐/民生，或AI仅顺带提及的"智能"营销标签稿）：novelty给0-2、impact给0-3，总分必然垫底无法入选
- 只有AI是主题或实质参与方（模型/算法/算力/AI产品/AI研究/AI政策/AI商业）才算AI资讯

评分约束：
- 旗舰模型的版本号发布（如GLM-5.3、GPT-5.6、Opus 5等）是新一代正式发布而非已知产品的小迭代：
  novelty按首发事件的口径给（正式发布+新能力=22-30），不得仅凭版本号像迭代就压进"实质新进展"档
  （2026-08-14事故：智谱GLM-5.3发布被当迭代判novelty=12，精评又误判可看，旗舰发布最终只得63分）
...
- 但同一篇论文/访谈/演讲被同一家媒体拆成多篇角度稿的，是同一event：同一学者/团队的"XX指出…""XX团队拆解…""XX验证…"系列标题就是典型拆稿（2026-08-21事故：魏华一篇论文被拆成5篇全部入库），必须聚成同一event
...
判档要求：
- 传闻/泄露类稿件（标题含"或将/据传/泄露/曝光/内部代号"且无官方确认的事实基础）：天花板"值得看"——
  传闻不是事实，再大的主体也要等官宣才配得上"重要"（2026-08-19事故："OpenAI或将发布GPT-6"传闻稿被评92分重磅）
...
- 一线AI实验室（OpenAI/Anthropic/Google/DeepSeek/智谱/月之暗面/字节/阿里等）旗舰模型的正式版发布
  （GPT-6/V4 Pro/Opus/GLM旗舰版等量级，注意x.3这类版本号也可能是旗舰代际而非小迭代，以正文参数量/对标表述为准），
  属"重要"档以上——模型能力竞赛是当前AI行业最核心赛道，旗舰发布直接影响开发者选型与行业走向；
  若官方公布性能对标/超越当前最强模型，且Agent/编程/推理等能力有实质提升，可至"重磅"（一天最多1条，多数天为0条）
- 发布事件稿件即使混入实测/体验环节（"刚刚，XX来了+上手演示"式结构），仍按发布事件本身定档，
  不得因含实测内容降为衍生/实测类
- 发布事件的"角色"是当事方首发（官方公告/官方博客）时，媒体转述稿（"XX媒体：某公司发布X"）参照主角定档，
  不因转述身份降档；只有"衍生报道"（适配/榜单/评论/复述）才受衍生稿档位天花板约束
